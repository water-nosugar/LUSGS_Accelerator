...
run_error_case(CfdLusgsDescriptor *desc, Problem *p, const Config *cfg)
{
    ...
    if (strcmp(cfg->error_case, "bad-token") == 0) {
        ...
        status = lusgs_launch_wait(desc, &token);
    }
    ...
    printf("%s\n", pass ? "LUSGS_STEP3_ERROR_PASS" :
                          "LUSGS_STEP3_ERROR_FAIL");
    return pass ? 0 : 1;
}
...
main(int argc, char **argv)
{
    ...
    uint64_t status = lusgs_launch_wait(&desc, &token);
    ...
    uint64_t expected_sub = cfg.stage == 'A' ? 0 : expected_patha;
    uint64_t expected_axpy = cfg.update_q ? cfg.lines * cfg.cells : 0;

    printf("lusgs.step3.stage = %c\n", cfg.stage);
    ...
    printf("lusgs.step3.cpuSubmitWaitCycles = %llu\n",
           (unsigned long long)step3_cycles);
    printf("lusgs.step3.launchToken = %llu\n", (unsigned long long)token);
    printf("lusgs.step3.waitStatus = %llu\n", (unsigned long long)status);
    ...
    printf("lusgs.step3_vs_reference.max_abs_error = %.17g\n",
           step3_ref.max_abs_error);
    printf("lusgs.step3_vs_reference.max_rel_error = %.17g\n",
           step3_ref.max_rel_error);
    printf("lusgs.step3_vs_reference.mismatch_count = %llu\n",
           (unsigned long long)step3_ref.mismatch_count);
    printf("lusgs.step3_vs_step2.max_abs_error = %.17g\n",
           step3_step2.max_abs_error);
    printf("lusgs.step3_vs_step2.max_rel_error = %.17g\n",
           step3_step2.max_rel_error);
    printf("lusgs.step3_vs_step2.mismatch_count = %llu\n",
