/*
 * test_cfd_pathc_za_outer_perf.c -- Path C ZA outer-product pipeline benchmark
 *
 * Path C:
 *   6 x lmat5_spm
 ...
 *
 * Conceptual outer-product step:
 *   outer[i][j] = A[i,k] * x[j]
 *
 * Default effective selected-column update:
 *   ZA[:,0] = A[:,0] * x[0]
 *   ZA[:,k] = ZA[:,k-1] + A[:,k] * x[k], k=1..4
 *
 * The final result is read from ZA[:,4].  No Z partial sums, no REDUCE5,
 * no fused MVM instruction, and no fixed ZA[:,0] accumulation path.
 */
...
run_pathc_za_outer_pingpong(double *ping_result, double *pong_result,
                            uint64_t iters)
{
    (void)enc_lmat5_spm;
    (void)enc_cfdsme_za_outer5_step;
    ...
    __asm__ __volatile__(
        ...
        ".long 0x01cf04a0\n\t" /* outer3 */
        ".long 0x008002a0\n\t" /* ping col4 */
        ".long 0x01d300a0\n\t" /* outer4: final in ZA[:,4] */
        ".long 0xc0c2800b\n\t" /* MOVA final PathC ZA[:,4] -> z11 */
        "st1d {z11.d}, p0, [x10]\n\t"
...
main(int argc, char **argv)
{
    ...
    printf("========================================================\n");
    printf("  CFD DSA -- PATH C SME-ZA OUTER-PIPE MVM BENCHMARK\n");
    printf("========================================================\n");
    printf("SPM layout per tile: 5x40B A^T columns + 1x40B vector\n");
    printf("Result store width: %dB (predicated st1d, first 5 FP64 lanes)\n",
           RESULT_STORE_BYTES);
    printf("PathC final result: selected-column outer pipeline ZA[0:4,4] -> Z result\n");
    ...
    run_pathc_za_outer_pingpong(heap_ping, heap_pong, bench_iters);
    m5_dump_stats_inline();
    sme_exit();

    pass &= verify_pair("Ping/Pong PathC ZA-outer MVM", heap_ping, expect_ping,
                        heap_pong, expect_pong, 1e-3);
    pass &= verify_pair("Benchmark PathC ZA-outer MVM", heap_ping, expect_ping,
                        heap_pong, expect_pong, 1e-3);
